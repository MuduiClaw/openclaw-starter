...
<!-- direct conversation = ask the user directly in conversation -->
<!-- Slash commands = natural language triggers via OpenClaw skill -->
...
If there's only one section, auto-select it. If there are multiple sections, use the direct conversation to ask which section the user wants to create a screen design for.
...
"I don't see a specification for **[Section Title]** yet. Please run **Shape Section** step first to define the section's requirements."

If data.json or types.ts don't exist:

"I don't see sample data for **[Section Title]** yet. Please run **Sample Data** step first to create sample data and types for the screen designs."
...
"Note: Design tokens haven't been defined yet. I'll use default styling, but for consistent branding, consider running **Design Tokens** step first."
...
"Note: An application shell hasn't been designed yet. The screen design will render standalone. Consider running **Design Shell** step first to see section screen designs in the full app context."
...
If the spec implies multiple views, use the direct conversation to confirm which view to build first:
...
- Run **Screenshot** step to capture a screenshot of this screen design for documentation
- If the spec calls for additional views, run **Design Screen** step again to create them
- When all sections are complete, run **Export Product** step to generate the complete export package"

If the spec indicates additional views are needed:

"The specification also calls for [other view(s)]. Run **Design Screen** step again to create those, then **Screenshot** step to capture each one."
