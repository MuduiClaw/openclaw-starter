<!-- Adapted from Agent OS (github.com/buildermethods/agent-os) -->
...
3. **Shaping/Planning** — In plan mode, building a spec, running **Shape Spec** step
...
Match index descriptions against the context. Ask the user to present suggestions:
...
First, ask the user to determine how to include the standards:
...
- **Check the index** — If suggestions seem wrong, run **Index Standards** step to rebuild
- **Keep standards concise** — Injected standards consume tokens; shorter is better

## Integration

This command is called internally by **Shape Spec** step to inject relevant standards during planning. You can also invoke it directly anytime you need standards in context.
